...
describe('DEPARTAMENTOS - Validación completa con gestión de errores y reporte a Excel', () => {
  ...
  function irADepartamentosLimpio() {
    ...
    return cy.url().then((currentUrl) => {
      const verificarPantallaCargada = () => {
        ...
        // Primero intentar buscar la tabla
      ...
      if (currentUrl.includes(DASHBOARD_PATH)) {
        ...
        cy.url({ timeout: 20000 }).should('include', DEPARTAMENTOS_PATH);
  ...
  function ejecutarCrearIndividual(casoExcel) {
    ...
    // Si el nombre contiene "pruebaXXX", reemplazar XXX con 3 números aleatorios (sin espacio)
    if (nombre.includes('pruebaXXX')) {
      const numerosAleatorios = Math.floor(100 + Math.random() * 900); // Genera número entre 100 y 999
      nombre = nombre.replace('pruebaXXX', `prueba${numerosAleatorios}`);
      cy.log(`Nombre con números aleatorios: "${nombre}"`);
    }
