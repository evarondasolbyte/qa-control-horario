...
describe('PRUEBAS USUARIO ADMIN - Validación completa con gestión de errores y reporte a Excel', () => {
  ...
  function verificarError500DespuesCrearJornadaSemanal(casoExcel, numero) {
    return cy.get('body', { timeout: 3000 }).then(
      ($body) => {
        //Blindaje: asegurar que $body existe y es "wrappable"
        const $safeBody = ($body && $body.length) ? $body : Cypress.$('body');

        const texto = ($safeBody.text() || '').toLowerCase();
        ...
        const tieneError500 =
          ...
          $safeBody.find('[class*="error-500"], [class*="error500"], [id*="error-500"]').length > 0;
      ...
      () => {
        // si no se pudo leer el body, NO marcar 500
        cy.log('No se pudo comprobar el body (timeout/navegación). NO se registra 500.');
        return cy.wrap(false);
      }
    );
  }
  ...
  function departamentosEditar(casoExcel) {
    ...
    return irAPantallaLimpio('/panelinterno/departamentos', 'Departamentos')
      .then(() => {
        ...
        cy.wait(500);
        // Buscar el botón "Editar" dentro de la primera fila visible de la tabla
        cy.get('.fi-ta-row:visible', { timeout: 10000 })
          .first()
          .within(() => {
  ...
  // Helper: Verificar error 500 después de abrir formulario de crear empleado
  function verificarError500AlAbrirFormulario(casoExcel) {
    return cy.get('body', { timeout: 5000 }).then(($body) => {
      ...
      if (tieneError500) {
        ...
        cy.registrarResultados({
          numero: parseInt(casoId.replace('TC', '')) || 0,
          nombre: nombreCaso,
          esperado: casoExcel.resultado_esperado || 'Comportamiento correcto',
          obtenido: 'ERROR 500: Error interno del servidor detectado en la página',
          resultado: 'ERROR',
          archivo,
          pantalla: 'Pruebas Usuario Admin'
        });
        ...
        return cy.document().then((doc) => {
          ...
          if (tieneError500EnDoc) {
            ...
            cy.registrarResultados({
              ...
              nombre: nombreCaso,
              esperado: casoExcel.resultado_esperado || 'Comportamiento correcto',
              obtenido: 'ERROR 500: Error interno del servidor detectado en el documento',
              resultado: 'ERROR',
              archivo,
              pantalla: 'Pruebas Usuario Admin'
            });
          ...
          cy.registrarResultados({
            ...
            nombre: nombreCaso,
            esperado: casoExcel.resultado_esperado || 'Comportamiento correcto',
            obtenido: 'ERROR: No se pudo abrir el formulario correctamente. La página no muestra el formulario esperado.',
            resultado: 'KO',
            archivo,
            pantalla: 'Pruebas Usuario Admin'
          });
      ...
      return cy.url({ timeout: 10000 }).then((url) => {
        if (!url.includes('/empleados/create')) {
          ...
          cy.registrarResultados({
            ...
            nombre: nombreCaso,
            esperado: casoExcel.resultado_esperado || 'Comportamiento correcto',
            obtenido: 'ERROR: No se pudo abrir el formulario correctamente. URL incorrecta.',
            resultado: 'KO',
            archivo,
            pantalla: 'Pruebas Usuario Admin'
          });
        ...
        const tieneFormulario = $body.find('form').length > 0 ||
          ...
          $body.find('label').filter((i, el) => {
            ...
            return textoLabel.includes('nombre') ||
      ...
      return cy.url().then((url) => {
        if (url.includes('error') || url.includes('500')) {
          ...
          cy.registrarResultados({
            ...
            nombre: nombreCaso,
            esperado: casoExcel.resultado_esperado || 'Comportamiento correcto',
            obtenido: 'ERROR 500: Error interno del servidor detectado en la URL',
            resultado: 'ERROR',
            archivo,
            pantalla: 'Pruebas Usuario Admin'
          });
        ...
        return cy.document().then((doc) => {
          ...
          cy.registrarResultados({
            ...
            nombre: nombreCaso,
            esperado: casoExcel.resultado_esperado || 'Comportamiento correcto',
            obtenido: 'ERROR: No se pudo abrir el formulario correctamente. No se pudo obtener el body de la página.',
            resultado: 'KO',
            archivo,
            pantalla: 'Pruebas Usuario Admin'
          });
  ...
  function empleadosCrearMinimo(casoExcel) {
    ...
    return irAPantallaLimpio('/panelinterno/empleados', 'Empleados')
      ...
      .then((resultado) => {
        // Si hubo error, no continuar con el resto del flujo
        if (resultado && resultado.huboError === true) {
          cy.log('Error detectado, no se continuará con el llenado del formulario');
          return cy.wrap(null);
        }
  ...
  function empleadosValidacionNombreObligatorio(casoExcel) {
    ...
    return irAPantallaLimpio('/panelinterno/empleados', 'Empleados')
      .then(() => {
        ...
        return verificarError500AlAbrirFormulario(casoExcel);
      })
      .then((resultado) => {
        if (resultado && resultado.huboError === true) {
          cy.log('Error detectado, no se continuará con el llenado del formulario');
